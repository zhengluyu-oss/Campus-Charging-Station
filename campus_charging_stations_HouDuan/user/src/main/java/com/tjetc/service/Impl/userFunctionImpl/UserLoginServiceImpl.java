...
public class UserLoginServiceImpl implements UserLoginService {
    ...
    public JsonResult login(LoginDTO loginDTO) {
        //校验数据是否为null 空字符串 甚至长度都要校验
        JsonResult jsonResult = checkUsernameAndPassword(loginDTO.getUsername(), loginDTO.getPassword());
        if (jsonResult.getState() != 0) {
            return jsonResult;
        }
        //查询
        User user = userMapper.selectByUsernameAndPassword(loginDTO.getUsername().trim(), loginDTO.getPassword().trim());
        if (user == null) {
            return JsonResult.fail("用户名或者密码错误");
        }
        user.setPassword(null);
